...
export default function Collection({ cards, addCard, removeCard }: CollectionProps) {
        ...
        <div className="card-grid" style={{ gridTemplateColumns: `repeat(${cardsPerRow}, 1fr)` }}>
          {groupedCards.map((card: any) => (
            <div className="card" key={card.id} onClick={() => setSelectedCard(card)}>
              ...
              {card.count > 1 && <div className="card-count">x{card.count}</div>}
              <button className="remove-button" onClick={() => removeCard(card)}>Remove</button>
            </div>
          ))}
        </div>
      )}
